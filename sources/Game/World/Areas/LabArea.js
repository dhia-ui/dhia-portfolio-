...
export class LabArea extends Area
{
    ...
    setImages()
    {
        ...
        // Placeholder texture (dark grey canvas) used when no image/video found
        this.images.createPlaceholderTexture = () =>
        {
        ...
        // Get resource and load (supports both video MP4 and KTX image)
        this.images.getResourceAndLoad = (key) =>
        {
            ...
            if(!resource)
            {
                ...
                // Find matching project to check for video property
                const project = labData.find(p => p.image === key)

                if(project && project.video)
                {
                    // Video texture
                    resource.isVideo = true
                    ...
                    {
                        resource.texture = this.images.createPlaceholderTexture()
                        resource.loaded = true
                        this.images.loadEnded(key)
                    }, { once: true })
    ...
    setScroller()
    {
        ...
        // Minis
        {
            ...
            for(const project of labData)
            {
                ...
                // Image (mini thumbnail — skip gracefully if no imageMini)
                {
                    ...
                    mini.startLoading = () =>
                    {
                        ...
                        {
                            // No thumbnail: show a coloured placeholder square
                            const placeholderColor = color('#2244aa')
                            const material = new MeshDefaultMaterial({
                                colorNode: placeholderColor,
                                hasWater: false,
                                hasLightBounce: false,
                            })
    ...
    changeProject(index = 0, direction = null, silent = false)
    {
        ...
        // Manage video playback: pause previous, play new
        if(this.navigation.previous && this.navigation.previous.video)
        {
            const prevResource = this.images.resources.get(this.navigation.previous.image)
            if(prevResource && prevResource.video)
                prevResource.video.pause()
        }
        const currentResource = this.images.resources.get(this.navigation.current.image)
        if(currentResource && currentResource.video)
        {
